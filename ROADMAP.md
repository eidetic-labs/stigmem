...
3. **Storage immutability stack** per [ADR-016](docs/adr/016-storage-immutability-enforcement.md) — L1 architectural append-only journal + projection tables has landed with projection schema, local-assert journal writes, embedding-status projection, zero remaining direct `facts` mutations in the tracked inventory, and read paths joined to validity/garden/quarantine/CID projections; remaining work adds L2 SQLite triggers, L3 CIDs (per [ADR-017](docs/adr/017-amendment-to-adr-011-cids-as-core.md)), L4 local hash chain, L5 Sigstore Rekor anchor, plus client/peer verification.
