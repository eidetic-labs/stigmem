...
| R-15 | T9-T1, T9-E1 | Instruction-scope write confers effective instruction-authoring authority over any agent using §21 lazy discovery | Low | Critical | High | §21.1, §3.5 | **Open** | Canonical feature analysis: [`experimental/lazy-instruction-discovery/security.md`](../../experimental/lazy-instruction-discovery/security.md). Partial mitigation shipped in #373: facts now carry `value.interpret_as`, local instruction-typed writes require `instruction:write`, and recall returns instruction-typed facts through the `instructions` channel. R-15 remains open until lazy-instruction admission/promotion policy and federation-inbound instruction handling land. |
...
|---|---|---|
| R-15 | Finish the instruction admission boundary: require explicit promotion policy for lazy-instruction use, quarantine or reject federation-inbound instruction-typed facts, and add protocol adversarial vectors. Local writes already require `instruction:write` as of #373. | High |
