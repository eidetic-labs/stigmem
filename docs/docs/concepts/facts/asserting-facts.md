...
If you are inside a Paperclip or Claude Code session with the Stigmem MCP server configured, call `assert_fact` directly — no `curl` required. See the [Paperclip connector guide](../../sdks/connectors/paperclip).
:::
...
See [Multi-Tenant Scoping](../../security/multi-tenancy) for the full isolation model and key provisioning guide.
