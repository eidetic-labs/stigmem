# Stigmem Design Partner Outreach — Draft Messages
...
**Subject:** Stigmem spec — your temporal edge model is in it, want to co-author federation?

Hi Daniel,

I built a spec called Stigmem — an open federated knowledge fabric for AI agents. While researching prior art, Zep's temporal edge model (`valid_at`/`invalid_at`) stood out as the cleanest solution to the "historical fact vs uncertain fact" conflation problem. I borrowed the pattern directly and cited Zep in the design decisions.

Stigmem is now v0.2 and going public. The federation section (§6) is the least-developed part — it's a sketch with the right intuitions but missing the operational detail that comes from actually running a multi-node system at scale.
...
**Subject:** Stigmem — federated agent memory spec, looking for co-authors on the federation section

Hi Charles,

I've been building a spec for federated agent memory called Stigmem. The core primitive is an atomic fact: `(entity, relation, value, source, timestamp, confidence, scope)` — immutable, provenance-first, contradiction-surfacing. It's what I'd want underneath a system like Letta if I needed memory to travel across agent boundaries and org boundaries without losing provenance.
...
**Subject:** Stigmem spec — interested in co-authoring the federation and namespace governance sections?

Hey,

Building an open spec for federated agent memory called Stigmem — atomic facts with provenance, scopes, contradiction detection, and eventually federation. Going public this week with v0.2.
...
- If any of these turn into calls, record key decisions as new facts in the Stigmem node and update the spec design decisions log (§7)
