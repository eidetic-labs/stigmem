...
See [Multi-Tenancy](../../build/guides/multi-tenancy) for the full model.
...
Keys issued for `beta-inc` see only facts written by `beta-inc` keys. See [Multi-Tenancy → Isolation in practice](../../build/guides/multi-tenancy#isolation-in-practice) for a cross-tenant verification example.
...
- [Multi-Tenancy](../../build/guides/multi-tenancy) — tenant isolation model
