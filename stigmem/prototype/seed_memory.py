"""Load example facts into a running Stigmem node.
...
def seed(url: str) -> None:
    src = "agent:example"
    print(f"Seeding Stigmem node at {url} ...")
    ...
    facts = [
        ...
        # Project state
        ("project:stigmem", "roadmap:status", sv("public-draft"), src, 1.0),
        ("project:stigmem", "roadmap:version", sv("v0.2"), src, 1.0),
        ("project:stigmem", "roadmap:license", sv("Apache-2.0"), src, 1.0),

        # Example contradiction pair — two agents disagree on budget
        ("project:stigmem", "budget:estimate_usd", {"type": "number", "v": 5000}, "agent:assistant", 0.7),
        ("project:stigmem", "budget:estimate_usd", {"type": "number", "v": 8000}, "agent:other", 0.6),
    ]
    ...
    print(f"  curl '{url}/v1/facts?entity=project:stigmem&include_contradicted=true'")
