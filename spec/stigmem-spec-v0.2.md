...
| `source`      | URI or opaque ID string           | Who asserted the fact. Examples: `agent:assistant`, `user:alice`, `system:loom`. |
...
```
// "Company A–Company B partnership requires board approval"
(entity="loom:rel:abc123", relation="rel:subject",  value={type:"ref", v:"company:a"})
(entity="loom:rel:abc123", relation="rel:object",   value={type:"ref", v:"company:b"})
...
fact (e.g., "product roadmap v1 was active in Q3 2024") should have `confidence=1.0` and
...
values, a contradiction exists. Stigmem nodes MUST surface contradictions rather than
...
EscalationPolicy {
  ...
  channel:      string      // e.g. "stigmem", "email", "slack" (v0.1: loom only)
...
## 6. Federation Handshake (Phase 3 sketch — co-authors wanted)

Federation is the highest-priority open question for v0.3. This sketch captures
the design intent so v0.1 implementations can be written federation-aware.
...
**If you have production opinions about this section, see the [design partner track](../CONTRIBUTING.md#design-partner-track).**
...
| `text` type added alongside `string` | Multi-paragraph rationale bodies don't fit `string`. Inline `text` avoids a content store for moderate-size bodies. `ref` for blobs >64 KB. |
| Reification via `loom:rel:` entities | Policies involving three or more entities have no natural primary entity. Reification is the RDF-proven pattern; encoding N-ary logic into a value type adds parsing complexity downstream. |
| `valid_until` field added to fact shape | Zep/Graphiti `valid_at`/`invalid_at` temporal edge model is the closest prior art. Separating temporal scope from `confidence` prevents "lower confidence to express a historical fact" anti-pattern. |

---

## 8. Open Questions (seeking design-partner input)
...
*v0.2 — public draft. Contributions welcome. See [CONTRIBUTING.md](../CONTRIBUTING.md).*
