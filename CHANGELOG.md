...
- **Grade-recovery plan to lift `/grade` from C (73) to B (≥80).** Tracked across four phased issues, each with explicit acceptance criteria and `/grade` row impact. Phase 0 (this entry, plus [PR #100](https://github.com/Eidetic-Labs/stigmem/pull/100)) files the issues and adds a `check_coverage_baseline.py` ratchet to prevent coverage drift while the phases land. Phases:
    ...
    - [#99 — Phase D](https://github.com/Eidetic-Labs/stigmem/issues/99): refactor 7 stretch CCN functions in `stigmem_node` (CCN 22–37). Re-evaluate scope after PR #109's god-file split, because the old plan bundled this work into that refactor to avoid conflicts. +2 (D-min, 5/7 refactored, lands at B=80) to +4 (D-full, 0 warnings, lands at B+2=82) to row 3.1. Requires `/security-review` if it still touches security-sensitive paths (`assert_fact_impl`, `_recall_impl`, `federation_ingest_tombstone_impl`).
...
- **Rename `adapters/openclaw/clawhub-skill/` → `adapters/openclaw/skill/` — done on `main`.** The directory's `clawhub-` prefix tripped two failure modes in the clawhub CLI: (a) the slug-from-directory-name inference failed ClawHub's protected-namespace check ("clawhub-*"), requiring `--slug stigmem-node` on every publish; (b) the display-name-from-directory-name inference produced "Clawhub Skill" instead of "Stigmem" when `--name` isn't passed (regressed twice in production: v1.0.3 fix, v1.0.6 re-regression, v1.0.7 fix). The rename removes both inference dependencies at the source. ClawHub skill bumped to v1.0.8 with a no-op-content changelog entry documenting the rename. The hard-coded `--name`/`--slug` flags in `.github/workflows/clawhub-publish.yml` (PR #82) become belt-and-suspenders post-rename rather than required workarounds.
