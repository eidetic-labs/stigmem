...
- **GHCR `:latest` was advancing on every push to main.** `publish.yml`'s `publish-node` job ran on push-to-main and pushed `:latest` + a per-sha tag to `ghcr.io/eidetic-labs/stigmem-node` regardless of whether the commit was a tagged release. After v0.9.0a1 four docs/CI commits drifted `:latest` away from the tagged digest — runtime binary identical (no Python source changes), but OCI `image.revision` label, SBOM, and cosign attestation all advanced. Fixed on `main`: `:latest` now advances only on tag push (matches the npm `latest` dist-tag convention); main pushes produce `:edge` + `:<sha>` instead. **Open follow-up:** repoint `:latest` to the v0.9.0a1 digest via `crane copy` or a `workflow_dispatch` job (registry retag; preserves digest + signatures).
- **Stale SDK version literal in `stigmem-py 0.9.0a1`.** `sdks/stigmem-py/src/stigmem/__init__.py` ships with `__version__ = "0.4.0"` and a docstring that says "Python client SDK for Stigmem (spec v0.4/v0.5)". Both are pre-reset artifacts that the v0.9.0a1 publish carried forward unfixed. Adopters running `import stigmem; print(stigmem.__version__)` see `0.4.0` instead of the actual installed version. v0.9.0a2's wheel refresh updates both literals (and adds a `check_sdk_version_consistency.py` CI check to prevent drift in future). Cosmetic but visible; not a security or installability issue.
