#!/usr/bin/env python3
"""CEO MEMORY.md → Muninn node migration script.

Reads the CEO's PARA-style memory files (MEMORY.md index + linked .md files),
converts each entry to a Muninn fact, and POSTs to the running node via HTTP.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Migrate CEO memory to Muninn node")
    parser.add_argument(
        "--memory-dir",
        required=True,
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Muninn node base URL (default: http://localhost:8765)",
    )
