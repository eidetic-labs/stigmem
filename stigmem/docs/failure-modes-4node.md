...
**Risk:** A sustained storm (e.g., agents continuously asserting conflicting values) can grow the `conflicts` table unboundedly. **No eviction or TTL on conflict records currently exists.** This is a spec v0.8 edge case to address (see D3 [ACM-63](/ACM/issues/ACM-63)).
...
These emerged from test iteration and are surfaced for [ACM-63](/ACM/issues/ACM-63):
