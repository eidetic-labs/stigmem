...
| R-15 | T3-E2, T9-T1, T9-E1 | Instruction-scope write confers effective instruction-authoring authority over any agent using §21 lazy discovery | Low | Critical | High | §21.1, §3.5 | **Open** | Canonical feature analysis: [`experimental/lazy-instruction-discovery/security.md`](../../experimental/lazy-instruction-discovery/security.md). Partial mitigation shipped in #373/#375/#377: facts now carry `value.interpret_as`, local instruction-typed writes require `instruction:write`, recall returns instruction-typed facts through the `instructions` channel, federation-inbound instruction-typed facts fail closed into quarantine, and protocol adversarial vectors cover forged instruction capability and inbound instruction quarantine. R-15 remains open until lazy-instruction admission/promotion policy and supported-adapter conformance evidence cover all consumers. |
...
| R-21 | Finish the cross-agent worm boundary: supported adapters must propagate sessions by default, and outbound replication must exclude transitively recalled facts. Same-session read/write graph isolation and source-fact provenance controls have landed. | High |
...
- R-05 is In review; R-15, R-16, R-17, R-18, R-21, R-22, and R-23 are Open and require implementation or documentation follow-up — see §8.2 for recommended mitigations.
