...
Hybrid recall — return the most salient facts for `query` within `token_budget`. Combines lexical (FTS5/BM25), dense-vector (ANN), and graph-traversal signals. See the [Recall guide](./recall.md) for details.
...
- [Recall guide](./recall.md) — hybrid recall, weight tuning, fast-path
- [Memory Cards guide](./memory-cards.md) — card lifecycle and divergence policy
- [Asserting facts](./asserting-facts.md) — detailed fact write patterns
- [Querying facts](./querying-facts.md) — structured predicate queries
