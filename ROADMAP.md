...
- [ ] **PR 4 series** — Plugin infrastructure + six cross-cutting plugins per ADR-011 as amended by [ADR-017](docs/adr/017-amendment-to-adr-011-cids-as-core.md):
  - **PR 4-INF.1** — Hook registry + protocol merged in [PR #137](https://github.com/Eidetic-Labs/stigmem/pull/137) on 2026-05-12. It adds the stable 22-hook surface, typed hook semantics, deterministic manual/core registration, minimum `PluginManifest` / `PluginContext` / capability APIs, hook-site wiring, audit/migration plumbing, test registry helpers, and the hook-firing benchmark gate. It intentionally does not include package discovery, dependency lifecycle, health polling, operator CLI, production signing/trust, or plugin author/operator docs.
  - **PR 4-INF.2–4** — Lifecycle/discovery + plugin migration lifecycle tracking, signing/trust hardening, and plugin author/operator docs.
