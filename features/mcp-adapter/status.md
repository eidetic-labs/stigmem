...
| Publication state | `published` - scoped package metadata, live protocol smoke, repo-local adapter security regressions, npm dry-run evidence, Codex CLI / Claude Code host UI smoke, Gemini CLI smoke with caveat, maintainer publication clearance, registry publication, and post-publish install verification are aligned for `@eidetic-labs/stigmem-mcp@0.1.0`. |
...
rejection, and credential/session boundaries. The `0.1.0` npm package is public
on the npm registry and installable from a clean consumer project. It was
published as a manual bootstrap without provenance; future package publications
use `.github/workflows/mcp-publish.yml` with npm Trusted Publisher/OIDC. Codex
CLI and Claude Code host UI smoke passes. Gemini CLI host UI smoke completed
with a final-response caveat. Continue.dev, Cursor, and Zed connector guides
remain experimental and unvalidated for `0.1.0`.
...
| Package alignment | Align package metadata and release artifact policy with the independent npm package line. | Complete for scoped `0.1.0` package metadata and OIDC release path | `adapters/mcp/package.json`; `docs/compatibility-matrix.yaml`; `.github/workflows/mcp-publish.yml` |
...
- npm provenance begins with the first Trusted Publisher release after the
  manual `0.1.0` bootstrap publication.
