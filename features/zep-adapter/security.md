...
| --- | --- | --- |
| Wrong session target | All push and pull calls require an explicit `session_id`. | `experimental/zep-adapter/src/stigmem_plugin_zep/adapter.py`; `experimental/zep-adapter/tests/test_zep_adapter.py` |
| API key handling | The Zep Cloud API key is read from `ZEP_API_KEY`; no key value is committed to the source tree. | `experimental/zep-adapter/README.md`; `experimental/zep-adapter/src/stigmem_plugin_zep/adapter.py` |
...
| Zep outage or failure | Query failures return an empty result set and log a warning; callers own retry/degradation policy. | `experimental/zep-adapter/src/stigmem_plugin_zep/adapter.py`; `experimental/zep-adapter/tests/test_zep_adapter.py` |
