...
- **Recall API** (`POST /v1/recall`) — hybrid ranking with MMR packing, memory cards, and confidence-weighted results. Spec §20.3.
...
| 9 | Recall & Graph — graph adjacency index, embeddings (sqlite-vec), recall API, **subscription primitive** | **In progress** |

Phase 7 is the substrate phase: Memory Garden, Source Attestation, and Intent Envelope form the foundation for the curator dashboard and the connector ecosystem. Phase 9 (v1.1-draft) adds the Recall & Graph primitive — the subscription primitive (`/v1/subscriptions`) shipped and is security-reviewed as of 2026-05-04. v1.0 GA (multi-tenant, OIDC/SSO, billing, public launch) follows Phase 7.
