...
| R-15 | T9-T1, T9-E1 | Instruction-scope write confers effective instruction-authoring authority over any agent using §21 lazy discovery | Low | Critical | High | §21.1, §3.5 | **Open** | Canonical feature analysis: [`experimental/lazy-instruction-discovery/security.md`](../../experimental/lazy-instruction-discovery/security.md). Partial mitigation shipped in #373/#375: facts now carry `value.interpret_as`, local instruction-typed writes require `instruction:write`, recall returns instruction-typed facts through the `instructions` channel, and federation-inbound instruction-typed facts fail closed into quarantine. R-15 remains open until lazy-instruction admission/promotion policy and protocol adversarial vectors land. |
| R-16 | T7-D2 | Admin key compromise enables tombstone DoS: irreversible entity suppression (§23) | Low | High | Medium | §23.2 | **Open** | Canonical feature analysis: [`experimental/tombstones/security.md`](../../experimental/tombstones/security.md). Protocol summary: tombstones are admin-only and auditable, but compromised admin credentials can still suppress critical entities until revocation and recovery procedures land. |
| R-17 | T7-I1 | Legal-hold tombstone + admin key compromise leaks pre-tombstone historical facts via `as_of` (§24) | Low | High | Medium | §24.3.2 | **Open** | Canonical feature analysis: [`experimental/tombstones/security.md`](../../experimental/tombstones/security.md). Protocol summary: `legal_hold: true` preserves historical data for admin-only time-travel reads, so legal-hold access needs narrower capability and regression coverage. |
...
| R-21 | T3-S1 (write-side) | Agent feedback-loop worm: an LLM-driven agent compromised via prompt injection (T3-S1) uses its own writer key to assert attacker-chosen facts; those facts replicate across the federation, infecting peer agents and propagating compromise | Medium | High | High | ADR-003, §22 (proposed) | **Open** | Partial mitigation shipped in #371: callers that set `Stigmem-Session` get per-session read/write graph isolation; same-scope writes after reads are rejected unless `write_mode="summarize_with_provenance"` carries source-fact provenance. R-21 remains open until session propagation is required by supported adapters and outbound replication exclusion for transitively recalled facts lands. The OpenClaw adapter the pre-reset spec ships with a handoff allowlist that defends against the handoff variant of this attack. |
...
| R-21 | Open | Protocol-level cross-cutting risk. Contributor notes now live in [`experimental/lazy-instruction-discovery/security.md`](../../experimental/lazy-instruction-discovery/security.md) and [`experimental/memory-garden-acl/security.md`](../../experimental/memory-garden-acl/security.md). |
...
|---|---|---|
| R-15 | Finish the instruction admission boundary: require explicit promotion policy for lazy-instruction use and add protocol adversarial vectors. Local writes require `instruction:write` as of #373; federation-inbound instruction-typed facts quarantine or fail closed as of #375. | High |
...
## 9. Residual Risk — v0.9.0a1 baseline

Pre-reset hardening is complete. The following risks remain in the v0.9.0a1 baseline (carried forward from pre-reset).
...
10. **Agent feedback-loop worm (R-21)** is the highest-priority new structural risk in v0.9.0a1. Stigmem now exposes `Stigmem-Session` read/write graph isolation: writes into scopes read earlier in the same session require `write_mode="summarize_with_provenance"` with source-fact provenance. R-21 remains open until supported adapters propagate sessions by default and outbound replication excludes transitively recalled facts. The OpenClaw adapter the pre-reset spec ships with a handoff allowlist that defends against one variant.
