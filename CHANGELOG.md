...
- **SECURITY.md** "Supported versions" table updated: `1.0.0-rc` retired; `0.9.0a1` listed as the current supported pre-release. v0.9.0a1 carries no stability guarantee — breaking changes during the the v0.9.0aN alpha series/B hardening window are expected and called out in this changelog.
...
- **Storage backend trait (the v0.9 attestation chain window)** — `StorageBackend` abstraction, `LibSQLBackend` for Turso/libSQL embedded-replica, `STIGMEM_STORAGE_BACKEND` env var, backend-parameterized test runner.
...
## [Unreleased] — Instruction migration + lazy instruction discovery (§21)
...
## [Unreleased] — the v0.9 attestation chain window — Storage adapter trait + libSQL/Turso backend
