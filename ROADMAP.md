...
3. **Storage immutability stack** per [ADR-016](docs/adr/016-storage-immutability-enforcement.md) — L1 architectural append-only journal + projection tables is underway with the projection schema, local-assert journal writes, embedding-status projection, and direct `facts` mutation inventory guard; remaining work burns down the tracked mutation paths, then adds L2 SQLite triggers, L3 CIDs (per [ADR-017](docs/adr/017-amendment-to-adr-011-cids-as-core.md)), L4 local hash chain, L5 Sigstore Rekor anchor, plus client/peer verification.
