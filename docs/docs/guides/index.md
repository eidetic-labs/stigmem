...
Phase 6 deliverables: installer, 4-node topology validation, v0.8 MCP tools, and design-partner pilots (ACM-60).

| Guide | Deliverable | Topic |
|-------|------------|-------|
| [4-Node Topology](./federation-4node) | D1 (ACM-61) | Full-mesh 4-node cluster setup, soak tooling, and failure modes |
| [Design Partner Notes](./design-partner-notes) | D5 (ACM-65) | Feedback themes from Zep, Letta, and Cognee pilots |
...
Pre-GA hardening items from the Phase 6 audit (ACM-100).

| Guide | Track F item | Status | Topic |
|-------|-------------|--------|-------|
| [Federation (soak results)](./federation#soak-results) | F1 (ACM-101) | ✅ shipped | Cursor-resume behavior verified under 4-node failure injection |
| [DB-Loss Recovery](./cursor-reset-recovery) | F2 (ACM-102) | ✅ shipped | `cursor-export` / `cursor-import` runbook — bounds re-pull cost after DB loss |
| [Async Lint/Decay APIs](./async-jobs) | F3 (ACM-103) | ✅ shipped | Job submission, status polling, and cancellation for lint and decay sweeps |
| F4–F6 | ACM-104–ACM-106 | 📋 tracked | IntentEnvelope, fuzzy entity resolver, Migration 004 scope columns |
...
|-------|-------------|-------|
| [Agent Keypairs](./agent-keypairs) | C1 (ACM-85) | Ed25519 keypair registration and node-enforced source attestation |
| [Human Key Issuance](./human-key-issuance) | C2 (ACM-86) | OIDC-backed key issuance for human principals |
| [Audit Log](./audit-log) | C3 (ACM-87) | End-to-end audit trail: principal → attested-source → fact-id |
...
|-------|-------------|-------|
| [Human Surface (Web UI)](./human-surface) | B2 (ACM-81) | Browser UI for curators, contributors, and consumers |
| [OIDC / SSO Integration](./oidc-sso) | B3 (ACM-82) | OIDC bridge — human IdP identity → scoped Stigmem API key |

---

## Track A — Memory Garden + Source Attestation

These guides cover the two new primitives added in spec v0.9. Spec drafts are in progress (ACM-75, ACM-76).

| Guide | Track A item | Topic |
|-------|-------------|-------|
| [Memory Gardens](./memory-gardens) | A1/A3 (ACM-75, ACM-77) | Named, ACL'd partitions above scope — roles, membership API, enforcement |
| [Source Attestation](./source-attestation) | A2 (ACM-76) | Signed source URIs bound to auth principal; node rejects mismatched sources |
