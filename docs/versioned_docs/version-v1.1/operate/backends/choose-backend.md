...
**Add encryption at rest:** install the `encryption,sqlcipher` extras and set `STIGMEM_AT_REST_ENCRYPTION=on`. See the [Key rotation](../runbooks/key-rotation) runbook for the full setup and rotation procedure.
