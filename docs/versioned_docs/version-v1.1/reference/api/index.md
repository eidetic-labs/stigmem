...
See the [Memory Cards guide](../../build/guides/memory-cards.md) for the full lifecycle, schema, and Python SDK usage.
...
Typically published automatically by `stigmem instruction migrate`. See the [Instruction Migration guide](../../build/guides/instruction-migration.md) for the full workflow.
...
See the [Instruction Migration guide](../../build/guides/instruction-migration.md) for the full dry-run → migrate → recall-instruction workflow.
...
Full documentation, idempotency semantics, and the tombstone lifecycle are in the [Instruction Migration guide](../../build/guides/instruction-migration.md).
...
Human principals can obtain an API key via OIDC. Configure `STIGMEM_OIDC_*` env vars; the node validates the JWT and maps role claims to `admin|writer|reader`. See [OIDC / SSO Integration](../../build/guides/oidc-sso).
...
The header is required when `STIGMEM_TENANT_HEADER_REQUIRED=true`. See [Multi-Tenant Scoping](../../build/guides/multi-tenancy).
