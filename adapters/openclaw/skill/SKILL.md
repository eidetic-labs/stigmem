...
metadata:
  openclaw:
    ...
    install:
      - kind: uv
        package: "stigmem-openclaw>=0.9.0a6,<1.0.0"
---
...
> **Alpha status.** This source copy is prepared for the v0.9.0a6 ClawHub
...
The install spec uses a version range (`stigmem-openclaw>=0.9.0a6,<1.0.0`) so compatible alpha-line updates are picked up automatically. A future alpha or beta release could change runtime behaviour.
...
pinned version tag (`:0.9.0a6`) or a `@sha256:<digest>` pin — the install guide
...
### v0.9.0a6 ClawHub artifact refresh
...
- Documentation: corrects the dependency-pinning section to the alpha line
  (`stigmem-openclaw>=0.9.0a6,<1.0.0`) and avoids claiming presentation-layer
