...
CID-based federation tamper detection is not yet enforced by default. The dual-format migration window (12 months from `v1_1_ga_at`) means CID-less facts from upgraded peers are currently accepted. Until §25 reaches GA:
...
**Status:** DRAFT normative (the v1.0 / v1.1 design window). §25.1–§25.8 carry MUST/SHOULD/MAY normative language.
...
Every new fact written MUST have a corresponding row in `fact_cid_aliases`. Existing facts (pre-v1.1) will have `cid IS NULL` until the backfill runner completes (§25.7.2).

#### §25.3.2 Dual-Addressing During Migration Window {#section-25-3-2}

During the migration window (until a deployment-configurable end-date; SHOULD be at least **12 months** from v1.1 GA):
...
3. A lookup by `fact_id` MUST behave identically to pre-v1.1 behavior.
...
A `cid` field MUST be added to the `FactRecord` schema (v1.1 the v1.0 / v1.1 design window
addition). The field is nullable only to accommodate pre-v1.1 records that
have not yet been backfilled (§25.7.2); all new facts MUST be written with a
non-null CID.

```
FactRecord (v1.1 addition):
  ...all prior fields...
  cid: string | null   // "sha256:<hex64>"; null only for pre-v1.1 records pending backfill
```
...
3. The federation envelope MUST include a `v1_1_ga_at` field (ISO 8601) indicating the origin node's v1.1 GA timestamp. Receiving nodes MUST reject facts with `cid: null` where `fact.created_at >= v1_1_ga_at`, emitting a `cid_mismatch` audit event (§22.3). Facts with `cid: null` where `fact.created_at < v1_1_ga_at` are accepted as legitimate pre-v1.1 legacy records. Nodes MUST NOT silently accept a fact with `cid: null` whose `created_at` postdates the declared `v1_1_ga_at`. This prevents malicious peers from bypassing tamper detection by stripping the `cid` field and claiming a post-Phase-13 fact is a legacy record.
...
assert_fact(fact: FactInput) → FactRecord
  // MUST compute and persist cid on every write (v1.1 forward)
...
Run Migration 013b (§25.3.1 DDL) as part of the the v1.0 / v1.1 design window upgrade. The full upgrade batch MUST apply migrations in order: 013a (tombstones, §23.5.3), then 013b (CID aliases, §25.3.1), then 013c (retraction log, §23.5.3). 013a and 013c MUST both be applied before 013b to maintain foreign-key integrity.
...
From v1.1 forward, `assert_fact` MUST:
