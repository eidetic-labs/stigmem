...
-- -------------------------------------------------------------------------
-- Peers + dependents: extend status CHECK for pending_tl_proof
-- SQLite cannot ALTER a CHECK constraint; rebuild the table and all
-- tables that hold a FOREIGN KEY reference to it (replication_cursors).
-- -------------------------------------------------------------------------
...
CREATE TABLE peers (
    ...
    status            TEXT NOT NULL CHECK(status IN ('pending_verification','active','rejected','revoked','pending_tl_proof')),
    established_at    TEXT,
...
CREATE TABLE federation_audit (
    ...
    event_type TEXT NOT NULL CHECK(event_type IN (
                    ...
                    'tl_proof_missing','tl_proof_verified','manifest_stored','manifest_refresh_failed'
               )),
    detail     TEXT,
    ts         TEXT NOT NULL
);

INSERT INTO peers               SELECT * FROM peers_old;
