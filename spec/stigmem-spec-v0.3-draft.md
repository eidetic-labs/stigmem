...
is meaningless if any caller can claim to be any entity. Stigmem v0.1 makes this
...
**v0.1 commitment:** Nodes MUST document their auth status in a `/.well-known/stigmem`
...
`stigmem:received_from` meta-fact:
```
(entity=<fact-id>, relation="stigmem:received_from", value={type:"ref", v:<peer-node-id>}, ...)
```
...
| `/.well-known/stigmem` endpoint | Enables auth-mode detection and peer discovery without out-of-band coordination |
