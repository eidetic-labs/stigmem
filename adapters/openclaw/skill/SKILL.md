...
`:latest` is fine for trying things out; for production swap to a pinned version tag (`:0.9.0a1`) or a `@sha256:<digest>` pin — see the [tag-selection guide](https://docs.stigmem.dev/en/latest/operators/deployment/install#image-tags).
