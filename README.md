...
> **Status: `v0.9.0a5` — preview alpha, pre-stable · Apache-2.0**
...
The features below are **implemented in code** but have **not yet completed adversarial validation** at v0.9.0a5. Read [LIMITATIONS.md](LIMITATIONS.md) for which deployment patterns are currently safe.
...
**Pre-release install via `pip`:** because v0.9.0a5 is a PEP 440 pre-release, `pip install stigmem` (default channel) will *not* pick it up. Use `--pre` to opt in to the alpha line, and pick the install scope appropriate to your role:
...
├── adapters/       ← adapter code (OpenClaw is experimental in v0.9.0a5; MCP deferred)
...
To report a vulnerability, use GitHub's private advisory process — **do not open a public issue**. See [SECURITY.md](SECURITY.md) for the full disclosure policy and the v0.9.0a5 security posture statement.
