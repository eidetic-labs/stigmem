...
| --- | --- | --- |
| Unbounded tool loop | The optional `run()` loop caps tool-call rounds. | `experimental/gemini-adapter/README.md`; `experimental/gemini-adapter/src/stigmem_plugin_gemini/adapter.py` |
...
| API key handling | Stigmem and Gemini keys are read from environment variables; no key values are committed to the source tree. | `experimental/gemini-adapter/README.md`; `experimental/gemini-adapter/src/stigmem_plugin_gemini/adapter.py` |
| Source attribution | Assertions use an explicit source entity, defaulting to `agent:gemini`. | `experimental/gemini-adapter/README.md`; `experimental/gemini-adapter/src/stigmem_plugin_gemini/adapter.py` |
